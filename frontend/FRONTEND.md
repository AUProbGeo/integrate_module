...
| F3.10 | **00 Workflow / Simple Workflow** (`/workflow`, own nav entry) — one form, one button: **a)** prior source radio (`prior_model_layered` default / `geoprior1d .xlsx` with workspace picker + **Upload .xlsx** → `api.save_upload`) + visible `N` only, other generator args in collapsed *Prior options*; **b)** system-file picker + collapsed *Forward options* (`method` ga-aem/anemone + forward advanced fields) → `ig.prior_data_em`; **c)** DATA picker + collapsed *Inversion options* + **Start inversion workflow**. All three steps run in **one** child process (`worker.run_workflow_job`, `api.start_workflow_job`); `_step_progress_cb` tags progress with `step/n_steps/step_label` so `run_panel` draws a **step bar (a/b/c chips)** above the per-step 0–100 % bar. Done panel lists prior / prior-data / posterior files. | ✅ | ✅ (`tests/test_workflow.py` 4 pass; curl on `examples/`: layered N=200 → GA-AEM → `ip_range=0:3` posterior in 2 s; uploaded `.xlsx` → geoprior1d N=300 → posterior; bad upload → inline error). **Fix:** forward/inversion fields are namespaced `fwd_*` / `inv_*` — the sections shared `N`/`Ncpu`/`parallel` and the form kept the last value, so the prior got `N=0`; `.gex` only (no `.stm`)  |
